...
export async function loadFromSupabase(): Promise<StoreData> {
  const [
    ...
    // Deze vier kwamen later dan de rest van het schema; een club die de nieuwe SQL nog
    // niet draaide kent ze niet, en dan mag de hele lading daar niet op stuklopen.
    // `lesson_groups` en `sick_leaves` zijn het verste in dat verhaal: de beheerder draait
    // die migraties zelf en heeft dat op dit moment misschien nog niet gedaan. Dan hoort de
...
const LEEGMAAKBAAR = [
  ...
  'working_hours', 'working_days', 'booking_periods', 'note_for_coach',
